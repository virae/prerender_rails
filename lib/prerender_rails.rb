module Rack
  class Prerender
    ...
    def should_show_prerendered_page(env)
      ...
      is_requesting_prerendered_page = true if @prerender_for_ip_addresses.is_a?(Array) && env['HTTP_X_FORWARDED_FOR'] && @prerender_for_ip_addresses.any? { |ip| env['HTTP_X_FORWARDED_FOR'].split(',').include?(ip) }
